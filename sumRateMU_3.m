function [SR_MU_LP_3]=sumRateMU_3(Pt,h1,h2,g1,NT,p_1_ini,p_2_ini,tolerance,num_iter)
...
Rp1=log2(1+abs(h1*p_1)^2/(abs(h1*p_2)^2+1));
Rp2=log2(1+abs(h2*p_2)^2/(abs(h2*p_1)^2+1));

C1e=log2(1+abs(g1*p_1)^2/(abs(g1*p_2)^2+1));
C2e=log2(1+abs(g1*p_2)^2/(abs(g1*p_1)^2+1));
...
while (loop)     
    ...
    constraints(5)=-rho_p_1+1e-6;
    constraints(6)=-rho_p_2+1e-6;
    constraints(7)=-rho_1_e+1e-6;
    constraints(8)=-rho_2_e+1e-6;
    ...
    Constraints=[constraints(1:17)<=0];
...
% if rho_p_1_ini<=1e-4 || rho_p_2_ini<=1e-4
%     SR_MU_LP_3=0;
% else
%     SR_MU_LP_3=value(Objective);
% end
...
if value_Rp1_C1e >= 0 && value_Rp2_C2e <= 0
    SR_MU_LP_3=value_Rp1_C1e;
end
if value_Rp1_C1e <= 0 && value_Rp2_C2e >= 0
    SR_MU_LP_3=value_Rp2_C2e;
end
if value_Rp1_C1e >= 0 && value_Rp2_C2e >= 0
    SR_MU_LP_3=value_Rp1_C1e+value_Rp2_C2e;
end
fprintf('MU_3 : rate=%1.3E \n',[SR_MU_LP_3]);
end