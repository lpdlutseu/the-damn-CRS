function SR_NOMA=sumRateNOMA_2(Pt,Pr,h1,h2,h3,g1,g2,NT,p_1_ini,p_2_ini,p_c_ini,theta_ini,ind_relay,tolerance,num_iter)
% user 1 as the common message,p_c=p_1,p_1=0
%% introduced parameters
T_1=abs(h1*p_2_ini)^2+1;
T_2=abs(h2*p_2_ini)^2+1;
T_3=abs(g1*p_2_ini)^2+1;
% T_4=abs(g1*p_1_ini)^2;
T_5=abs(g1*p_2_ini)^2;
T_c_1=abs(h1*p_1_ini)^2;
T_c_2=abs(h2*p_1_ini)^2;
T_c_3=abs(g1*p_1_ini)^2;
% I_1=T_1;
...
Rc1=theta*log2(1+(h1*p_1)'*(h1*p_1)/((h1*p_2)'*(h1*p_2)+1))+(1-theta)*log2(1+Pr*h3'*h3);
Rc2=theta*log2(1+(h2*p_1)'*(h2*p_1)/((h2*p_2)'*(h2*p_2)+1));
Cce=theta*log2(1+(g1*p_1)'*(g1*p_1)/((g1*p_2)'*(g1*p_2)+1))+(1-theta)*log2(1+Pr*g2'*g2);
Rp2=theta*log2(1+(h2*p_2)'*(h2*p_2));
C2e=theta*log2(1+(g1*p_2)'*(g1*p_2)/((g1*p_1)'*(g1*p_1)+1));
...
Objective=min(alpha_c_1,alpha_c_2)+alpha_p_2-alpha_2_e-alpha_c_e;  
...
%     constraints(17)=-rho_p_1;
    constraints(18)=-rho_p_2+1e-6;
%     constraints(19)=-rho_1_e;
    constraints(20)=-rho_2_e+1e-6;
    constraints(21)=-rho_c_1+1e-6;
    constraints(22)=-rho_c_2+1e-6;
    constraints(23)=-rho_c_e+1e-6;

    constraints(24)=p_2'*p_2+p_1'*p_1-Pt; % Quadratic scalar (real, 24 variables)
...
%     constraints(34)=((h1*p_2)'*(h1*p_2))+1-2*real(p_1_ini'*h1'*h1*p_1)/rho_p_1_ini+((h1*p_1_ini)'*(h1*p_1_ini))*rho_p_1/(rho_p_1_ini^2);
    constraints(35)=1-2*real(p_2_ini'*h2'*h2*p_2)/rho_p_2_ini+((h2*p_2_ini)'*(h2*p_2_ini))*rho_p_2/(rho_p_2_ini^2);  
    constraints(36)=((h1*p_2)'*(h1*p_2))+1-2*real(p_1_ini'*h1'*h1*p_1)/rho_c_1_ini+((h1*p_1_ini)'*(h1*p_1_ini))*rho_c_1/(rho_c_1_ini^2);
    constraints(37)=((h2*p_2)'*(h2*p_2))+1-2*real(p_1_ini'*h2'*h2*p_1)/rho_c_2_ini+((h2*p_1_ini)'*(h2*p_1_ini))*rho_c_2/(rho_c_2_ini^2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% rewrite begin %%%%%%%%%%%%%%%%
...
%     constraints(38)=-(((g1*p_c_ini)'*(g1*p_c_ini)+(g1*p_2_ini)'*(g1*p_2_ini))*(rho_1_e-2*rho_1_e_ini)+rho_1_e+2*rho_1_e_ini*real(p_c_ini'*g1'*g1*p_c+p_2_ini'*g1'*g1*p_2))+(g1*p_1)'*(g1*p_1);
    constraints(39)=-(((g1*p_1_ini)'*(g1*p_1_ini))*(rho_2_e-2*rho_2_e_ini)+rho_2_e+2*rho_2_e_ini*real(p_1_ini'*g1'*g1*p_1))+(g1*p_2)'*(g1*p_2);
    constraints(40)=-(((g1*p_2_ini)'*(g1*p_2_ini))*(rho_c_e-2*rho_c_e_ini)+rho_c_e+2*rho_c_e_ini*real(p_2_ini'*g1'*g1*p_2))+(g1*p_1)'*(g1*p_1);
...
if value_Rc_Cce<=0 && value_Rp2_C2e<=0
    SR_NOMA=0;
end
if value_Rc_Cce>=0 && value_Rp2_C2e<=0
...
if value_Rc_Cce<=0 && value_Rp2_C2e>=0
    SR_NOMA=value_Rp2_C2e;
end
if value_Rc_Cce>=0 && value_Rp2_C2e>=0
    SR_NOMA=value_Rc_Cce+value_Rp2_C2e;
end

theta_opt=value(theta);

fprintf('NOMA_2 : theta= %1.0f,rate= %1.3E \n',[theta_opt SR_NOMA]);
end