function SR_RS_2=sumRateRS_2(Pt,Pr,h1,h2,h3,g1,g2,NT,p_1_ini,p_2_ini,p_c_ini,theta_ini,ind_relay,tolerance,num_iter)
...
Objective=min(alpha_c_1,alpha_c_2)-alpha_c_e+alpha_p_1-alpha_1_e;  
...
%     constraints(19)=(rho_c_2_ini^2)*(((h2*p_1)'*(h2*p_1))+((h2*p_2)'*(h2*p_2))+1)-2*real(p_c_ini'*h2'*h2*p_c)*rho_c_2_ini+((h2*p_c_ini)'*(h2*p_c_ini))*rho_c_2;
    ...
    constraints(31)=-0.5*(theta_ini-beta_1_e_ini)*(theta-beta_1_e)+0.25*(theta_ini-beta_1_e_ini)^2+0.25*(theta+beta_1_e)^2-alpha_1_e;

    % Quadratic scalar (real) and Signomial scalar(real)
%     constraints(32)=(rho_p_1_ini^2)*(((h1*p_2)'*(h1*p_2))+1)-2*real(p_1_ini'*h1'*h1*p_1)*rho_p_1_ini+((h1*p_1_ini)'*(h1*p_1_ini))*rho_p_1;
    constraints(32)=((h1*p_2)'*(h1*p_2))+1-2*real(p_1_ini'*h1'*h1*p_1)/rho_p_1_ini+((h1*p_1_ini)'*(h1*p_1_ini))*rho_p_1/(rho_p_1_ini^2);
    constraints(33)=-(((g1*p_c_ini)'*(g1*p_c_ini)+(g1*p_2_ini)'*(g1*p_2_ini))*(rho_1_e-2*rho_1_e_ini)+rho_1_e+2*rho_1_e_ini*real(p_c_ini'*g1'*g1*p_c+p_2_ini'*g1'*g1*p_2))+(g1*p_1)'*(g1*p_1);

    constraints(34)=exp(beta_p_1*log(2))-(1+rho_p_1);    % Nonlinear scalar (real)   
    constraints(35)=1+rho_1_e-(1+log(2)*(beta_1_e-beta_1_e_ini))*exp(beta_1_e_ini*log(2));    % Linear scalar (real) 
...
SR_RS_2=value(Objective);
fprintf('RS : theta_2= %1.0f,rate_2= %1.3E,ratio_2=%1.3f \n',[theta_opt SR_RS_2 ratio_comm]);
end