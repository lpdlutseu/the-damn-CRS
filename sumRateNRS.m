function SR_NRS=sumRateNRS(Pt,h1,h2,g1,NT,p_1_ini,p_2_ini,p_c_ini,tolerance,num_iter)
...
%     constraints(9)=-alpha_c_e;
    ...
    constraints(25)=(rho_p_1_ini^2)*(((h1*p_2)'*(h1*p_2))+1)-2*real(p_1_ini'*h1'*h1*p_1)*rho_p_1_ini+((h1*p_1_ini)'*(h1*p_1_ini))*rho_p_1;
    constraints(26)=(rho_p_2_ini^2)*(((h2*p_1)'*(h2*p_1))+1)-2*real(p_2_ini'*h2'*h2*p_2)*rho_p_2_ini+((h2*p_2_ini)'*(h2*p_2_ini))*rho_p_2;  
    constraints(27)=(rho_c_1_ini^2)*(((h1*p_1)'*(h1*p_1))+((h1*p_2)'*(h1*p_2))+1)-2*real(p_c_ini'*h1'*h1*p_c)*rho_c_1_ini+((h1*p_c_ini)'*(h1*p_c_ini))*rho_c_1;
    constraints(28)=(rho_c_2_ini^2)*(((h2*p_1)'*(h2*p_1))+((h2*p_2)'*(h2*p_2))+1)-2*real(p_c_ini'*h2'*h2*p_c)*rho_c_2_ini+((h2*p_c_ini)'*(h2*p_c_ini))*rho_c_2;  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% rewrite begin %%%%%%%%%%%%%%%%
    % Signomial scalar(real)
    constraints(29)=-(((g1*p_c_ini)'*(g1*p_c_ini)+(g1*p_2_ini)'*(g1*p_2_ini))*(rho_1_e-2*rho_1_e_ini)+rho_1_e+2*rho_1_e_ini*real(p_c_ini'*g1'*g1*p_c+p_2_ini'*g1'*g1*p_2))+(g1*p_1)'*(g1*p_1);
    constraints(30)=-(((g1*p_c_ini)'*(g1*p_c_ini)+(g1*p_1_ini)'*(g1*p_1_ini))*(rho_2_e-2*rho_2_e_ini)+rho_2_e+2*rho_2_e_ini*real(p_c_ini'*g1'*g1*p_c+p_1_ini'*g1'*g1*p_1))+(g1*p_2)'*(g1*p_2);
    constraints(31)=-(((g1*p_1_ini)'*(g1*p_1_ini)+(g1*p_2_ini)'*(g1*p_2_ini))*(rho_c_e-2*rho_c_e_ini)+rho_c_e+2*rho_c_e_ini*real(p_1_ini'*g1'*g1*p_1+p_2_ini'*g1'*g1*p_2))+(g1*p_c)'*(g1*p_c);

    % Nonlinear scalar (real)
    constraints(32)=exp(beta_p_1*log(2))-(1+rho_p_1); 
    constraints(33)=exp(beta_p_2*log(2))-(1+rho_p_2);
    constraints(34)=exp(beta_c_1*log(2))-(1+rho_c_1); 
    constraints(35)=exp(beta_c_2*log(2))-(1+rho_c_2);    
    
    % Linear scalar (real)   
    constraints(36)=1+rho_1_e-(1+log(2)*(beta_1_e-beta_1_e_ini))*exp(beta_1_e_ini*log(2));
    constraints(37)=1+rho_2_e-(1+log(2)*(beta_2_e-beta_2_e_ini))*exp(beta_2_e_ini*log(2)); 
    constraints(38)=1+rho_c_e-(1+log(2)*(beta_c_e-beta_c_e_ini))*exp(beta_c_e_ini*log(2));  
    constraints(39)=beta_c_e-beta_c_1;
    constraints(40)=beta_c_e-beta_c_2;
    Constraints=[constraints(10:40)<=0];
...
%     ops = sdpsettings('solver','fmincon','fmincon.Algorithm','sqp','fmincon.MaxIter',Inf,'fmincon.MaxFunEvals',Inf,'verbose',0);
    ...
    Objective=min(beta_c_1,beta_c_2)-beta_c_e+beta_p_1-beta_1_e+beta_p_2-beta_2_e;
...
SR_NRS=max(R_c-C_c_e,0)+max(R_p_1-C_1_e,0)+max(R_p_2-C_2_e,0);
fprintf('NRS : rate= %1.3E,ratio=%1.3f \n',[SR_NRS ratio_comm]);
end